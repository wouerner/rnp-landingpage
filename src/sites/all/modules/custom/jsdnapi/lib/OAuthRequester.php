...
class OAuthRequester extends OAuthRequestSigner
{
	 ...
	 */
	function __construct ( $request, $method = null, $params = null, $body = null, $files = null, $headers_params = null )
	{
            $this->headers_params = $headers_params;
	...
	protected function curl_raw ( $opts = array() )
	{
		...
		else
		{
			...
			$request_method = explode('/', $url);
                        if(isset($request_method[5]) && $request_method[5] != 'getImage'){
                            $header[]         = 'accept: application/json';
                        }
                        elseif($this->headers_params != ''){
                            $header[]         = 'accept: */*';
                            $header[]         = 'Content-Type: '.$this->headers_params;
                        }
			if ($method == 'POST')
			{
				if (!$has_content_type)
