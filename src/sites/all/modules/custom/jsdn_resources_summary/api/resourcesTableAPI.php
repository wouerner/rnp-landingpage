...
$provider = urlencode($_GET['provider']);
$regions = $_GET['regions'];
$index = $_GET['length']/10;
$sortColumn = 'null';
if(isset($_GET['order'])){
...
$api_url = JSDN_OAUTH_HOST."/api/v2/resources/list-resources";
$resources = jsdn_resources_chart_list_resources($api_url, $resources_body);
$output = json_decode($resources, TRUE);
$output['draw'] =  $draw;
...
function jsdn_resources_chart_list_resources($api_url, $resources_body) {
    ...
    $curlConfig = array(
        ...
        CURLOPT_POST           => true,
        CURLOPT_POSTFIELDS     => $resources_body
    );
    if ($isProxied){
