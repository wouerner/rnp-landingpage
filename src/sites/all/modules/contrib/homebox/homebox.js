    ...
    attach: function (context) {
      ...
      if ($homebox.length > 0) {
        ...
        Drupal.homebox.$columns.sortable({
		  ...
		  receive:
                    function(event,ui) {
                        var refreshFn=$(ui.item).find('.widgetDiv').attr("fname");
					  // window[refreshFn]();
                        var chart_id = $(ui.item).find(".chart_daily_trend").attr("id");
                        var chart_select_id = $(ui.item).find(".chart_filter").attr("id");
                        var chart_filter_count_id = $(ui.item).find(".chart_filter_count").attr("id");
                        var chart_type = $('#' + chart_select_id).val();
                        var chart_count = $('#' + chart_filter_count_id).val();
                        if ((typeof(chart_count) === "undefined") || (chart_count === null)){
                            chart_count = 5;
                        }
                        if(chart_id){
                            window[chart_id](chart_count, chart_type);
                        }
  ...
  Drupal.homebox.maximizeBox = function (icon) {
    ...
    if (!$(portlet).hasClass('ui-sortable-helper')) {
      ...
      if ($(portlet).hasClass('portlet-maximized')) {
       ...
       if(pageType != 'inventory'){
            var chart_id = $(portlet).find(".chart_daily_trend").attr("id");
            var chart_select_id = $(portlet).find(".chart_filter").attr("id");
            var chart_filter_count_id = $(portlet).find(".chart_filter_count").attr("id");
            var chart_type = $('#' + chart_select_id).val();
            var chart_count = $('#' + chart_filter_count_id).val();
            if ((typeof(chart_count) === "undefined") || (chart_count === null)){
                chart_count = 5;
        ...
        /*
          var refreshFn=$(portlet).find('.widgetDiv').attr("fname");
		window[refreshFn]();
                */
    }
  };
    ...
    attach: function (context) {
      $('.homebox-portlet:not(.homebox-processed)', context).addClass('homebox-processed').each(function () {
        ...
        $.each($portlet.attr('class').split(' '), function (key, a) {
		  ...
		  if(jj="ffffff"){
		  bcolor="d9e7e7";
		  }else{bcolor=a.substr(14);}
		  
		  
            $portletHeader.attr('style', 'background: #' + a.substr(14));
            $portlet.find('.homebox-portlet-inner').attr('style', 'border: 1px solid #' + bcolor);
          }
        });
